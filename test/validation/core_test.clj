...
(deftest test-latitude
  (testing "valid latitude"
    (are [number]
      (is (valid? ((latitude :latitude) {:latitude number})))
      -90.0 90 0 90 90.0))
  (testing "invalid latitude"
    (are [number]
      (let [record ((latitude :latitude) {:latitude number})]
        (is (not (valid? record)))
...
(deftest test-longitude
  (testing "valid longitude"
    (are [number]
      (is (valid? ((longitude :longitude) {:longitude number})))
      -180.0 180 0 180 180.0))
  (testing "invalid longitude"
    (are [number]
      (let [record ((longitude :longitude) {:longitude number})]
        (is (not (valid? record)))
...
;;     (are [location]
;;       (is (valid? ((location :location) {:location location})))
;;       (point 4326 0 0)
;;       (point 4326 -180 0)
;;       (point 4326 180 0)
;;       (point 4326 0 90)
...
;;       (let [record ((location :location) {:location location})]
;;         (is (not (valid? record)))
...
;;       {:latitude ["must be between -90.0 and 90.0."]})
;;     (is (= (error-messages ((location :location) nil))
;;            (error-messages ((location :location) {}))))))

(deftest test-location
  (testing "valid locations"
    (are [obj]
      (is (valid? ((location :location) {:location obj})))
      (point 4326 0 0)
      (point 4326 -180 0)
      (point 4326 180 0)
      (point 4326 0 90)
  ...
  (testing "invalid locations"
    (are [obj messages]
      (let [record ((location :location) {:location obj})]
        (is (not (valid? record)))
        (is (= (error-messages-on record :location) messages)))
      nil
      ["is not a location"]
      ...
      (point 4326 180.1 0)
      ["longitude must be between -180.0 and 180.0."]
      (point 4326 180.1 90.1)
      ["longitude must be between -180.0 and 180.0." "latitude must be between -90.0 and 90.0."])))
